...
public class Board {
	...
	private Stack<SudokuAction> actionsTaken = new Stack<>();
	private Stack<SudokuAction> actionsUndone = new Stack<>();
	...
	public void startBoard(String numbers) {
		...
		for(int x = 0; x < 9;x++) {
			for(int i = 0; i < 9; i++) {
					...
					String startValue = Character.toString(this.numbers.charAt(counter));
	...
	public void emptyStack(Stack<SudokuAction> stack) {
		if(stack.size()<1) {
			throw new SudokuBoardException("Kan ikke angre/gjenta før trekk er gjort");
		}
	}
	
	public void undo() {
		emptyStack(actionsTaken);
		String value = "";
		if(actionsTaken.size()>1) {
			for (int i=0; i<actionsTaken.size()-1; i++) {
				if(actionsTaken.peek().getColumn() == actionsTaken.get(i).getColumn() && actionsTaken.peek().getRow() == actionsTaken.get(i).getRow()) {
					value = " " + actionsTaken.get(i).getValue() + " ";
				}
				else {
					value = " . ";
				}
		...
		board.get(actionsTaken.peek().getRow()-1).set(actionsTaken.peek().getColumn()-1, myCell);
		actionsUndone.push(actionsTaken.pop());
	}
	
	public void redo() {
		emptyStack(actionsUndone);
		playerMove(actionsUndone.peek().getRow()-1,actionsUndone.peek().getColumn()-1,actionsUndone.peek().getValue());
		actionsTaken.push(actionsUndone.pop());
	}
	...
	public static void main(String[] args) {
		...
		board.getInput("1,1,7");
		//board.getInput("4,5,1");
		//board.getInput("4,5,2");
		//board.getInput("4,5,9");
		...
		board.save("C:/Users/figo_/Documents/tdt4100-2018-master2/git/tdt4100-2018/ovinger/src/game/file.txt");
		//board.load("C:/Users/figo_/Documents/tdt4100-2018-master2/git/tdt4100-2018/ovinger/src/game/file.txt");
		System.out.println(board.toString());
	...
	public void save(String fileName) {
		...
		SaveSudoku.save(fileName, SS.getBoardString(board));
	}
	
	public void load(String fileName) {
		startBoard(SaveSudoku.load(fileName));
	}
	...
	public void getInput(String in) {
		...
		toString();
		actionsTaken.add(new SudokuAction(in));
		for (int i=0; i<actionsUndone.size(); i++) {
			actionsUndone.pop();
		}
	}
